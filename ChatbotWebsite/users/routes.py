...
def account():
    ...
    if form.validate_on_submit():
        if form.picture.data:
            old_picture = current_user.profile_image.decode()
    ...
    # Decode the image if it's stored as bytes in the database
    profile_image = current_user.profile_image
    if isinstance(profile_image, bytes):
        profile_image = profile_image.decode()

    # Optional: Set a default image if profile_image is None or empty
    if not profile_image:
        profile_image = "default.jpg"


    # Pass the variable to your template
